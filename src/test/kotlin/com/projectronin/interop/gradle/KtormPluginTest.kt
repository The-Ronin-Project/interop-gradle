...
class KtormPluginTest {
    ...
    fun `includes Ktorm dependencies`() {
        ...
        mainSourceSet.compileClasspath.assertHasJars("ktorm-core", "ktorm-support-mysql")
    }

    @Test
    fun `includes TestContainers dependencies`() {
        val testSourceSet = project.sourceSets().getByName(SourceSet.TEST_SOURCE_SET_NAME)
        testSourceSet.compileClasspath.assertHasJars("junit-jupiter", "mysql")
        testSourceSet.runtimeClasspath.assertHasJars("mysql-connector-java")
    }

    @Test
    fun `includes Database Rider dependencies`() {
        val testSourceSet = project.sourceSets().getByName(SourceSet.TEST_SOURCE_SET_NAME)
        testSourceSet.compileClasspath.assertHasJars("rider-junit5")
    }

    @Test
    fun `includes Liquibase dependencies`() {
        val testSourceSet = project.sourceSets().getByName(SourceSet.TEST_SOURCE_SET_NAME)
        testSourceSet.compileClasspath.assertHasJars("liquibase-core")
        testSourceSet.runtimeClasspath.assertHasJars("liquibase-slf4j", "snakeyaml")
    }
}
